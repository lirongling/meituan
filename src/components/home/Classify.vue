<template>
  <div>
    <div class="container">
      ...
      <div v-for="(item,index) in menuList" :key="index" class="container-item flex" @mouseenter="onMouse(item.child)" @mouseleave="outMouse">
        <div>
          <i class="iconfont " :class="`icon-${item.type}`" :style="{color: item.color}"></i>
          <span class="category-text">{{item.name}}</span>
        </div>
        <div class="return"><i class="iconfont icon-iconfontzhizuobiaozhun19 return"></i></div>
      </div>
      <div class="container-de" :class="{'show-de' : showDe>0}" @mouseleave="mouseShow(-1)" @mouseenter="mouseShow(1)">
        <div v-for="(item,index) in menuDetails" :key="index">
          <div class="detail-title flex">
            <div class="details-name">{{item.title}}</div>
            <div class="detail-more">更多<span class="iconfont icon-iconfontzhizuobiaozhun19 "></span></div>
          </div>
          <div class="detail-content ">
            <div class="details-text" v-for="(items,index) in item.child">{{items}}</div>
          </div>
        </div>
      </div>
    </div>
